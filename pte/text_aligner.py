...
def find_align_block(score_matrix):
    ...
    if page_size<para_size/2+1: return -1,-1,0
    maxscore = 0
    r = (-1,-1)
    for i in range(page_size):
        #for j in range(i+para_size/2, i+para_size*2):  ### para_size/2~para_size*2
        for j in range(i+para_size-2, i+para_size+2):
            if j>page_size: break
            mat = score_matrix[:,i:j+1]
...
def paragraph_align(sentlst1,titlelst1,sentlst2,titlelst2):
    ...
    for title in titlesetlst:
        ...
        start,end,score = find_align_block(mat)
        para_pos_lst.append((idxs[0],idxs[-1],start,end,score))
    return para_pos_lst

def para_match_print(para_pos_lst,sentlst1,titlelst1,sentlst2,titlelst2,f):
    for pos in para_pos_lst:
        ori_start, ori_end, align_start, align_end, score = pos
        out = '[origin]: \n=='+titlelst1[ori_start]+'==\n'
        out+=unicode.join(u'\n', sentlst1[ori_start:ori_end+1])
        if align_start>=0:
            out+='\n[align]: \n'+unicode.join(u'\n', sentlst2[align_start:align_end+1])+'\nscore: '+str(score/(ori_end-ori_start+1))+'\n\n'
        else:
            out+='\n[not aligned]\n\n'
        f.write(out.encode('utf8'))
...
### date standardize
def compareDates(s1,s2):
    try:
        dt1 = dateparse(s1)
        dt2 = dateparse(s2)
    except ValueError:
        return False
    ss1 = dt1.strftime('%Y-%m-%d')
    ss2 = dt2.strftime('%Y-%m-%d')

    return ss1==ss2

def entity_sim(ent1,ent2):
    if ent1.label_!=ent2.label_: return 0
    if ent1.label_=='DATE': return compareDates(ent1.text, ent2.text)
    char_ngram1 = ngramStrExtract(ent1.text,1)+ngramStrExtract(ent1.text,2)+ngramStrExtract(ent1.text,3)+ngramStrExtract(ent1.text,4)
    char_ngram2 = ngramStrExtract(ent2.text,1)+ngramStrExtract(ent2.text,2)+ngramStrExtract(ent2.text,3)+ngramStrExtract(ent2.text,4)
    return item_match_score(char_ngram1,char_ngram2)
