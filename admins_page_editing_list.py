...
def get_user_contribs(user, ucsize=50):
    ...
    for i in range(ucsize / uclimit + 1):
        query = '/w/api.php?action=query&list=usercontribs&format=json&ucuser=' + user + '&uclimit=' + str(
            uclimit) + '&ucprop=ids|title|timestamp|sizediff|comment|size|tags'
...
def edit_feature1(contrib):
    sdiff, size, comment, page_title = int(contrib['sizediff']), int(contrib['size']), contrib['comment'].encode(
        'ascii', errors='ignore').lower(), contrib['title'].encode('ascii', errors='ignore').lower()
    feat = np.zeros((11,))
...
def get_blocked_list(num=100, starttime=None):
    query = '/w/api.php?action=query&list=blocks&bkprop=id|user|by|timestamp|expiry|reason|range|flags&bklimit=' + str(
        num) + '&format=json'
...
# format from scraped admin start dates
# assume format input of: ['2007','March','14']
def date_formatter(date_list):
    date_dict = {'Jan': '1', 'Feb': '2', 'March': '3', 'Apr': '4', 'May': '5', 'June': '6', 'July': '7', 'Aug': '8',
                 'Sept': '9', 'Oct': '10', 'Nov': '11', 'Dec': '12'}
    return date_list[0] + "-" + date_dict[date_list[1]] + "-" + date_list[2] + "T10:31:38Z"
...
if __name__ == '__main__':
    ...
    contribs = []
    admins_list = []
    pickle_in = open("admin_start_dict.pickle", "rb")
    ...
    for idx, admin in enumerate(admins_list):
        ...
        for c in contribs:
            if (c["title"].find("User") == -1 and c["title"].find("File:") == -1 and c["title"].find("Talk:") == -1 and
                    c['sizediff'] > 2):
                ...
                if (time_diff_in_sec(c["timestamp"], date_formatter(admin_start_dict[admin.strip()])) < 0):
                    ...
                    user_articles["after_admin"].append(to_utf8(c["title"]))
                # except:
                #    print "Date time formatting error! Skipping user..."
                #    continue
