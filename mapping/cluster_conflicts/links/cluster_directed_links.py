...
if __name__ == '__main__':
    ...
    aggregator_vector = csc_matrix((data, indices, indptr), shape=(n, midway))
    aggregator_vector = aggregator_vector[first_half_indices, :]

    testing_vector = secondary_matrix[first_half_indices, :] + secondary_matrix[:, first_half_indices].transpose()
    testing_vector = testing_vector[:, first_half_indices]
    test_aggreg = testing_vector.dot(aggregator_vector)

    aggregated_vector = secondary_matrix[second_half_indices, :] + secondary_matrix[:, second_half_indices].transpose()
    aggregated_vector = aggregated_vector[:, first_half_indices]
    aggregation = aggregated_vector.dot(aggregator_vector)
    aggregation = aggregation.todense()

    threshold_scores = []
    for i, cluster in enumerate(hierarchical_clusters[:midway]):
        cluster_scores = []
        for item in cluster:
            score = test_aggreg[first_half_reverse_indices[item['index']], i]
            cluster_scores.append(score)
        threshold_scores.append(np.median(cluster_scores) * 0.1)
    ...
    for i, second_half_index in enumerate(second_half_indices):
        ...
        max_score = np.max(aggregation[i])
        cluster_of_choice = np.argmax(aggregation[i])

        if max_score > threshold_scores[cluster_of_choice]:
